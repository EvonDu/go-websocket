...
//WebSocket处理函数
func (t *BaseWebSocket) Handler(ws *websocket.Conn) {
	//执行事件客户端连接
	t.onConnect(ws)
	//轮询
...
//连接客户端
func (t *BaseWebSocket) onConnect(ws *websocket.Conn) {
	//执行事件客户端连接
...
//断开客户端
func (t *BaseWebSocket) onClose(ws *websocket.Conn) {
	//移除连接
...
//接受客户端信息
func (t *BaseWebSocket) onMessage(ws *websocket.Conn, data string) {
	//触发事件
...
//添加客户端连接事件
func (t *BaseWebSocket) AddOnConnect(event func(ws *websocket.Conn)){
	t.EventOnConnect = append(t.EventOnConnect,event)
}

//添加客户端断开事件
func (t *BaseWebSocket) AddOnClose(event func(ws *websocket.Conn)){
	t.EventOnClose = append(t.EventOnClose,event)
}

//添加接收信息事件
func (t *BaseWebSocket) AddOnMessage(event func(ws *websocket.Conn, data string)){
	t.EventOnMessage = append(t.EventOnMessage,event)
}

//添加发送信息事件
func (t *BaseWebSocket) AddOnPublish(event func(data string)){
	t.EventOnPublish = append(t.EventOnPublish,event)
}

//广播信息
func (t *BaseWebSocket) Publish(message string){
	//广播发送
	for i:=0;i<len(t.Connects);i++ {
